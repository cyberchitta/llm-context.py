...
def test_highlight_generation(language, extension, code, expected_highlights, tagger):
    source = Source(f"test_file.{extension}", code)
    highlights = generate_highlights(tagger, [source])

    assert len(highlights) == 1, f"Expected exactly one highlight for {language}"
    assert highlights[0]["rel_path"] == f"test_file.{extension}", f"Path mismatch for {language}"
    assert "highlights" in highlights[0], f"Highlights key missing for {language}"

    actual_highlights = highlights[0]["highlights"].strip()
