...
class FileIgnorer:
    ...
    def ignore(self, path: str, is_dir: bool) -> bool:
        ...
        ignored = any(self._match_pattern(pattern, path, name, is_dir) for pattern in self.ignore_patterns)

        # Then check negation patterns
        if ignored:
            negated = any(self._match_pattern(pattern, path, name, is_dir) for pattern in self.negation_patterns)
            return not negated
